class Skill < ApplicationRecord
  ...
  def self.search(params)
    ...
    skills = skills.where("lower(unaccent(skills.formatted_address)) LIKE ?", "%#{params[:location].downcase}%") if params[:location].present?
    skills = skills.where("lower(unaccent(skills.name)) LIKE ?", "%#{params[:skill].downcase}%") if params [:skill].present?
    return skills
  end
